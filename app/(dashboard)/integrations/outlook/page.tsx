...
import OutlookMcpTools from "@/components/dashboard/outlook-mcp-tools";
...
export default function OutlookIntegrationPage() {
  ...
  const outlook = statuses[PROVIDER];
  const connected = outlook?.status === STATUS_CONNECTED;
  ...
  return (
    <>
      ...
      <PageHeader
        title="Outlook"
        description="Manage your Outlook connection and run its MCP tools."
      />
      ...
      {!isLoaded ? (
        ...
        <div className="space-y-5">
          ...
          >
            <div className="flex flex-wrap gap-x-12 gap-y-4">
              ...
              <Stat
                ...
                value={
                  outlook?.connected_at
                    ? new Date(outlook.connected_at).toLocaleDateString()
                    : "—"
                }
              />
              <Stat label="Tool catalog" value="agent365.svc.cloud.microsoft" />
            </div>
            {errors[PROVIDER] && (
          ...
          <Card
            ...
            subtitle="Loaded live from Microsoft's official Work IQ MCP servers. Tools without an explicit user argument act on your own mailbox and calendar."
          >
            {connected ? (
              <OutlookMcpTools />
            ) : (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Connect Outlook to load and run its tools.
              </p>
            )}
          </Card>
        </div>
